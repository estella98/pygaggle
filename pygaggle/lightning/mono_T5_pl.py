...
class MonoT5Transformer(BaseTransformer):
    """
    A training module for MonoT5 See BaseTransformer for the core options.
    """
    ...
    def __init__(self, hparams):
        ...
        module = import_module("tasks") #need to update
        try:
            summarization_task_clazz = getattr(module, hparams.task_type)
            self.token_classification_task: TokenClassificationTask = token_classification_task_clazz()
        except AttributeError:
            raise ValueError(
                f"Task {hparams.task_type} needs to be defined as a sunmnarizationTask subclass in {module}. "
